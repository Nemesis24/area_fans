    ...
    async def async_step_user(self, user_input=None):
        ...
        for area_name, fans in fans_by_area.items():
            ...
            fan_dict = {fan["id"]: f"{fan['name']} ({fan['id']})"
                        for fan in fans}
            data_schema[vol.Optional(display_name, default=[])] = cv.multi_select(fan_dict)
...
class AreaFansOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        for area_name, fans in fans_by_area.items():
            ...
            data_schema[vol.Optional(display_name, default=area_excluded)] = cv.multi_select(fan_dict)
